  ...
  return (
    <div style={{ display: "grid", gap: 18 }}>
      <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
        ...
        {runId && <Link to={`/onboarding/${runId}`} style={{ fontSize: 14 }}>View onboarding details</Link>}
      </div>

      <section style={{ border: "1px solid rgba(0,0,0,0.12)", borderRadius: 18, padding: 18, background: "white" }}>
        ...
        {!loading && !err && run && (
          <>
            <div style={{ marginTop: 12, fontSize: 14, opacity: 0.9 }}>
              <b>Status:</b> {run.status} · <b>Started:</b> {fmt(run.started_at)} · <b>Finished:</b> {fmt(run.finished_at)}
            </div>
            ...
            <div style={{ marginTop: 14, overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
                ...
                <tbody>
                  ...
                  {steps.length === 0 && (
                    <tr>
                      <td style={td} colSpan={4}>No steps logged yet.</td>
                    </tr>
                  )}
                </tbody>
              </table>
